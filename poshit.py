...
def poshit():
  ...
  while nonce <= max:
    blockHash = sha256(sha256(nonce.to_bytes(32, "big") + height.to_bytes(4, "big") + bytes.fromhex(targethex) + version.to_bytes(4, "big") + prevHash + merkle + timestamp.encode("utf-8")).digest()).hexdigest()
